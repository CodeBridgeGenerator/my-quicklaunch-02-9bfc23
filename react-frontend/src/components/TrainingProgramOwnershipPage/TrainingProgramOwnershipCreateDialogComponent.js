...
const TrainingProgramOwnershipCreateDialogComponent = (props) => {
    ...
    const validate = () => {
        ...
        const error = {};
            ...
            if (_.isEmpty(_entity?.organisationType)) {
                error["organisationType"] = `OrganisationType field is required`;
                ret = false;
            }
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            organisationName: _entity?.organisationName,organisationType: _entity?.organisationType,ownershipType: _entity?.ownershipType,
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info TrainingProgramOwnership created successfully" });
...
<div className="col-12 md:col-6 field mt-5">
            <span className="align-items-center">
                <label htmlFor="organisationType">OrganisationType:</label>
                <InputText id="organisationType" className="w-full mb-3 p-inputtext-sm" value={_entity?.organisationType} onChange={(e) => setValByKey("organisationType", e.target.value)}  required  />
            </span>
            <small className="p-error">
            {!_.isEmpty(error["organisationType"]) ? (
              <p className="m-0" key="error-organisationType">
                {error["organisationType"]}
              </p>
            ) : null}
          </small>
            </div>
