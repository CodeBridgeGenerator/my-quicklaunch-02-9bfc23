...
import OrganisationDatatable from "./OrganisationDataTable";
...
import OrganisationFakerDialogComponent from "./OrganisationFakerDialogComponent";
import OrganisationSeederDialogComponent from "./OrganisationSeederDialogComponent";
...
const OrganisationPage = (props) => {
    ...
    const filename = "organisation.csv";

    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("organisation");
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("organisation").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Organisation", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
        ...
        const promises = data.map((e) => client.service("organisation").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
        ...
        <div className="mt-5">
            <div className="grid">
                <div className="col-6 flex justify-content-start">
                    <h3 className="mb-0 ml-2">Organisation </h3>
                </div>
                <div className="col-6 flex justify-content-end">
                    <>
                    <Button label="add" loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="organisation-add-button"/>
            ...
            <div className="grid align-items-center">
                <div className="col-12" role="organisation-datatable">
                    <OrganisationDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>
            ...
            <OrganisationEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <OrganisationCreateDialogComponent entity={newRecord} show={showCreateDialog} onHide={() => setShowCreateDialog(false)} onCreateResult={onCreateResult} />
            <OrganisationFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <OrganisationSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(OrganisationPage);
