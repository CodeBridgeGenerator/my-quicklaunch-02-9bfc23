...
organisationNo: _entity?.organisationNo,
        };

        setLoading(true);
        try {
            
        const result = await client.service("organisation").patch(_entity._id, _data);
        props.onHide();
        props.alert({ type: "success", title: "Edit info", message: "Info organisation updated successfully" });
    ...
    return (
        <Dialog header="Edit Organisation" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="organisation-edit-dialog-component">
