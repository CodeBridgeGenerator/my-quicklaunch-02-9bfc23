...
import organisationFakerFactory from "./organisationFakerFactory";
...
const OrganisationFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = organisationFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("organisation").create(elem));
...
export default connect(mapState, mapDispatch)(OrganisationFakerDialogComponent);
